//
//  NewProviderViewController.swift
//  ContabilidadAjos
//
...
class NewProviderViewController: UIViewController, UIAlertViewDelegate {
    ...
    func save() {
        if identifier.text != "" && name.text != "" && direction.text != "" {
            let provider = Provider(identifier: identifier.text!, name: name.text!, direction: direction.text!, city: city.text!, province: province.text!)
            provider.save()
